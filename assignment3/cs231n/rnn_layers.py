...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    prodX = np.dot(x, Wx) # N x H
    prodH = np.dot(prev_h, Wh) # N x H
    next_h = np.tanh(prodX + prodH + b) # N x H
...
def rnn_step_backward(dnext_h, cache):
    ...
    x = cache['x']
    prev_h = cache['prev_h'] # N x h
    Wx = cache['Wx'] # d x h
    Wh = cache['Wh'] # h x h
    tanh = cache['tanh']

    dTanh = 1.0 - tanh**2 # N x h
    dTanhandOut = dTanh * dnext_h # N x h
    
    dprev_h = np.dot(dTanhandOut, Wh.T)
    dWh = np.dot(dTanhandOut.T, prev_h).T
    dx = np.dot(dTanhandOut, Wx.T)
    dWx = np.dot(dTanhandOut.T, x).T
    db = np.sum(dTanhandOut, axis=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for num in range(T):
      ...
      sample = x[:, num, :]
      h0, currCache = rnn_step_forward(sample, h0, Wx, Wh, b)
      h[:, num, :] = h0
      cache.append(currCache)
...
def rnn_backward(dh, cache):
    ...
    for num in range(T-1, -1, -1):
      
      dh0 += dh[:, num, :]
      dxCurr, dh0, dWxCurr, dWhCurr, dbCurr = rnn_step_backward(dh0, cache[num])
      dx[:, num, :] = dxCurr
      dWh += dWhCurr
      dWx += dWxCurr
      db += dbCurr
