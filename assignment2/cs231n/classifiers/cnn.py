...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        num_chanels = input_dim[0]

        W1 = np.random.normal(scale=weight_scale, size=(num_filters, num_chanels, filter_size, filter_size))
        b1 = np.zeros((num_filters, 1))

        first_dim = num_filters * input_dim[1] * input_dim[2] // 4
        W2 = np.random.normal(scale=weight_scale, size=(first_dim, hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        affine1_out, cache_affine1 = affine_forward(pool_out, W2, b2)
        ...
        regLoss = (np.sum(W1 ** 2) + np.sum(W2**2) + np.sum(W3**2)) * self.reg / 2
        loss += regLoss
        ...
        if db1.shape == (db1.shape[0], ): # fast реализация возвращает (3,) вместо (3, 1), что мешает корректной работе
          db1 = db1[:, None]
        dW1 += W1 * self.reg
